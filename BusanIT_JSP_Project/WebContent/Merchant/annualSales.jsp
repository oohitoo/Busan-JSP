...
<script type="text/javascript">
	function no(year, month) {
		location.href = "monthlySales.jsp?year=" + year + "&month=" + month;

	}
	function previous(year) {
		var int_year = Number(year);
		previous_year = int_year - 1
		location.href = "annualSales.jsp?year=" + previous_year;
	}
	function next(year) {
		var int_year = Number(year);
		next_year = int_year + 1
		location.href = "annualSales.jsp?year=" + next_year;
	}
	function main() {
							...
							<tr>
								...
								<td>
									<div style="float: right; margin-right: 50px;">
										<button type="button" class="btn btn-primary"
											onclick="javascript:next('<%=year%>')">다&nbsp;음&nbsp;년&nbsp;도</button>
									</div>
								</td>
							</tr>
						</table>
							...
							<tr>
								<td colspan="5" align="center">
									<button type="button" class="btn btn-primary"
										onclick="javascript:previous('<%=year%>')">이&nbsp;전&nbsp;년&nbsp;도</button>
									<button type="button" class="btn btn-primary"
										onclick="javascript:main()">메&nbsp;인&nbsp;화&nbsp;면</button>
									<button type="button" class="btn btn-primary"
										onclick="javascript:next('<%=year%>')">다&nbsp;음&nbsp;년&nbsp;도</button>
								</td>
							</tr>
